 ...
 * src/lib/schemas.ts
 *
 * Role: Zod validation schemas — single source of truth for form validation.
 * TypeScript types are derived from these schemas using z.infer<>.
 *
 * Design Decision: Zod v4 API is used (z.string().min() etc. are unchanged,
 * but z.coerce.date() → validate against plain string for HTML date inputs).
 * NIK validated as exactly 16 numeric digits via regex.
 */
